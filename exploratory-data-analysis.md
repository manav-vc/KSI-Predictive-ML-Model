...
tags:
  ...
  - open-data
datasets:
  - toronto_police_service_public_safety_data_portal
---
...
* **`DATE`** (Top 5 Dates with most collisions):
    ...
    * *Note: Specific dates with high counts might indicate local events or data anomalies.*
...
* **`STREET1`** (Top 5 primary streets):
    ...
    * DUFFERIN ST: 294
    * EGLINTON AVE E: 288
    * *Note: Identifies frequently occurring collision locations.*
...
* **`STREET2`** (Top 5 secondary streets for collisions):
    ...
    * FINCH AVE E: 126
    * EGLINTON AVE E: 123
    * *Note: Complements STREET1 for intersection analysis.*
...
* **`OFFSET`** (Top 5 offsets from a reference point):
    ...
    * 10 m South o: 59
    * 5 m East of: 55
    * *Note: Highly granular, may be challenging to use directly without aggregation or conversion to numerical distance.*
...
* **`ROAD_CLASS`** (Road classification where collision occurred):
    ...
    * Local: 865
    * Expressway: 164
    * *Note: Major arterials account for the vast majority of collisions.*
...
* **`DISTRICT`** (City district where collision occurred):
    ...
    * Scarborough: 4270
    * North York: 3788
    * *Note: Provides geographical segmentation within Toronto.*
...
* **`ACCLOC`** (Collision location type):
    ...
    * *Note: Intersections are the most common collision location. Significant missing values present.*
...
* **`NEIGHBOURHOOD_158`** (City of Toronto Neighbourhood name (new) - Top 5 names):
    ...
    * South Riverdale: 353
    * South Parkdale: 304
    * *Note: Provides finer-grained location information than District.*
...
* **`NEIGHBOURHOOD_140`** (City of Toronto Neighbourhood name (old) - Top 5 names):
    ...
    * *Note: Provides alternative neighbourhood definitions; check for overlap/redundancy with _158 versions.*
...
* **`DIVISION`** (Toronto Police Service Division - Top 5 divisions):
    ...
    * D22: 1413
    * D32: 1328
    * *Note: Another geographical indicator, potentially useful for resource allocation or local policy analysis.*
...
* **`TRAFFCTL`** (Traffic Control Type):
    ...
    * *Note: 'No Control' and 'Traffic Signal' are the dominant categories.*
...
* **`VISIBILITY`** (Environment Condition):
    ...
    * *Note: Most collisions occur under clear visibility.*
...
* **`LIGHT`** (Light Condition):
    ...
    * Dusk: 253
    * Dusk, artificial: 253
    * *Note: Majority occur in daylight, but a significant portion in dark conditions, with or without artificial light.*
...
* **`RDSFCOND`** (Road Surface Condition):
    ...
    * Other: 147
    * Slush: 102
    * *Note: Most collisions on dry surfaces, followed by wet surfaces.*
...
* **`ACCLASS`** (Classification of Accident - **Your Target Variable**):
    ...
    * *Note: This is the primary target for your prediction. Observe the significant class imbalance between Non-Fatal Injury and Fatal collisions.*
...
* **`IMPACTYPE`** (Initial Impact Type):
    ...
    * Rear End: 1804
    * SMV Other: 1465
    * *Note: Pedestrian collisions are the most frequent impact type.*
...
* **`INVTYPE`** (Involvement Type):
    ...
    * Vehicle Owner: 1638
    * Cyclist: 822
    * *Note: Drivers are the most frequently involved party.*
...
* **`INVAGE`** (Age of Involved Party - Top 5 age ranges):
    ...
    * 30 to 34: 1450
    * 35 to 39: 1382
    * *Note: Contains 'unknown' values which need handling. This is a categorical representation of age.*
...
* **`INJURY`** (Severity of Injury):
    ...
    * Minimal: 1160
    * Fatal: 976
    * *Note: This is closely related to your target variable `ACCLASS`. The count for 'Fatal' here (976) differs from `ACCLASS` 'Fatal' (2670), which suggests careful reconciliation is needed for your target definition.*
...
* **`INITDIR`** (Initial Direction of Travel):
    ...
    * North: 3201
    * Unknown: 526
    * *Note: Relatively even distribution across cardinal directions, with some 'Unknown' values.*
...
* **`VEHTYPE`** (Type of Vehicle):
    ...
    * *Note: Automobiles are the most common vehicle type involved.*
...
* **`MANOEUVER`** (Vehicle Manoeuver):
    ...
    * *Note: 'Going Ahead' is by far the most frequent maneuver. Significant missing values present.*
...
* **`DRIVACT`** (Apparent Driver Action):
    ...
    * Improper Turn: 614
    * Other: 529
    * *Note: 'Driving Properly' is the top action, which might seem counter-intuitive for an accident; this category could include situations where the other party was at fault, or where the 'action' leading to the collision isn't well captured.*
...
* **`DRIVCOND`** (Driver Condition):
    ...
    * *Note: Majority are 'Normal', but 'Inattentive' is a significant factor. Also contains 'Unknown'.*
...
* **`PEDTYPE`** (Pedestrian Crash Type - detail - Top 5):
    ...
    * *Note: Highly detailed, applies only when a pedestrian is involved. Very high missing value count for records without pedestrians.*
...
* **`PEDACT`** (Pedestrian Action - Top 5):
    ...
    * Other: 240
    * *Note: Similar to PEDTYPE, applicable only to pedestrian-involved collisions.*
...
* **`PEDCOND`** (Condition of Pedestrian - Top 5):
    ...
    * Other: 84
    * *Note: Condition of pedestrian at the time of collision.*
...
* **`CYCLISTYPE`** (Cyclist Crash Type - detail - Top 5):
    ...
    * *Note: Highly detailed, applies only when a cyclist is involved. Very high missing value count.*
...
* **`CYCACT`** (Cyclist Action - Top 5):
    ...
    * Lost control: 42
    * *Note: Actions of the cyclist at the time of collision.*
...
* **`CYCCOND`** (Condition of Cyclist - Top 5):
    ...
    * Other: 12
    * *Note: Condition of the cyclist at the time of collision.*
...
* **`EMERG_VEH`**: Yes: 49
    * *Note: Very low count for emergency vehicles, indicating rarity. Consider if this feature is useful or too sparse.*
* **`PASSENGER`**: Yes: 7183
